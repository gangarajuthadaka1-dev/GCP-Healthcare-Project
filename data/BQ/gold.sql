...
CREATE TABLE IF NOT EXISTS `project-819f30a6-533e-44c8-a6e.gold_dataset.provider_charge_summary` (
...
# truncate table
TRUNCATE TABLE `project-819f30a6-533e-44c8-a6e.gold_dataset.provider_charge_summary`;

# insert data
INSERT INTO `project-819f30a6-533e-44c8-a6e.gold_dataset.provider_charge_summary`
SELECT 
    CONCAT(p.firstname, ' ', p.LastName) AS Provider_Name,
    d.Name AS Dept_Name,
    SUM(t.Amount) AS Amount
FROM `project-819f30a6-533e-44c8-a6e.silver_dataset.transactions` t
LEFT JOIN `project-819f30a6-533e-44c8-a6e.silver_dataset.providers` p 
    ON SPLIT(p.ProviderID, "-")[SAFE_OFFSET(1)] = t.ProviderID
LEFT JOIN `project-819f30a6-533e-44c8-a6e.silver_dataset.departments` d 
...
# CREATE TABLE
CREATE TABLE IF NOT EXISTS `project-819f30a6-533e-44c8-a6e.gold_dataset.patient_history` (
    Patient_Key STRING,
    FirstName STRING,
    LastName STRING,
    Gender STRING,
...
# TRUNCATE TABLE
TRUNCATE TABLE `project-819f30a6-533e-44c8-a6e.gold_dataset.patient_history`;

# INSERT DATA
INSERT INTO `project-819f30a6-533e-44c8-a6e.gold_dataset.patient_history`
SELECT 
    p.Patient_Key,
    p.FirstName,
    p.LastName,
    ...
    c.PayorType
FROM `project-819f30a6-533e-44c8-a6e.silver_dataset.patients` p
LEFT JOIN `project-819f30a6-533e-44c8-a6e.silver_dataset.encounters` e 
    ON SPLIT(p.Patient_Key, '-')[OFFSET(0)] || '-' || SPLIT(p.Patient_Key, '-')[OFFSET(1)] = e.PatientID
LEFT JOIN `project-819f30a6-533e-44c8-a6e.silver_dataset.transactions` t 
    ON SPLIT(p.Patient_Key, '-')[OFFSET(0)] || '-' || SPLIT(p.Patient_Key, '-')[OFFSET(1)] = t.PatientID
LEFT JOIN `project-819f30a6-533e-44c8-a6e.silver_dataset.claims` c 
...
# CREATE TABLE
CREATE TABLE IF NOT EXISTS `project-819f30a6-533e-44c8-a6e.gold_dataset.provider_performance` (
    ProviderID STRING,
    FirstName STRING,
    LastName STRING,
...
# TRUNCATE TABLE
TRUNCATE TABLE `project-819f30a6-533e-44c8-a6e.gold_dataset.provider_performance`;

# INSERT DATA
INSERT INTO `project-819f30a6-533e-44c8-a6e.gold_dataset.provider_performance`
SELECT 
    pr.ProviderID,
    pr.FirstName,
    pr.LastName,
...
FROM `project-819f30a6-533e-44c8-a6e.silver_dataset.providers` pr
LEFT JOIN `project-819f30a6-533e-44c8-a6e.silver_dataset.encounters` e 
...
# CREATE TABLE
CREATE TABLE IF NOT EXISTS `project-819f30a6-533e-44c8-a6e.gold_dataset.department_performance` (
    Dept_Id STRING,
...
# TRUNCATE TABLE
TRUNCATE TABLE `project-819f30a6-533e-44c8-a6e.gold_dataset.department_performance`;

# INSERT DATA
INSERT INTO `project-819f30a6-533e-44c8-a6e.gold_dataset.department_performance`
SELECT 
    d.Dept_Id,
...
FROM `project-819f30a6-533e-44c8-a6e.silver_dataset.departments` d
LEFT JOIN `project-819f30a6-533e-44c8-a6e.silver_dataset.encounters` e 
...
CREATE TABLE IF NOT EXISTS `project-819f30a6-533e-44c8-a6e.gold_dataset.financial_metrics` AS
SELECT 
    COUNT(DISTINCT t.Transaction_Key) AS TotalTransactions,
    SUM(t.Amount) AS TotalBilledAmount,
    SUM(t.PaidAmount) AS TotalPaidAmount,
...
CREATE TABLE IF NOT EXISTS `project-819f30a6-533e-44c8-a6e.gold_dataset.payor_performance` AS
SELECT 
    c.PayorID,
    c.PayorType,
    COUNT(DISTINCT c.Claim_Key) AS TotalClaims,
...
FROM `project-819f30a6-533e-44c8-a6e.silver_dataset.claims` c
